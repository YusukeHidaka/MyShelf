class OriginalBooksController < ApplicationController
  ...
  def create
    ...
    create_colored_book(original_book)
  end
  ...
  def create_colored_book(original_book)
    # ranking がない場合作成
    ...
    if colored_book = ColoredBook.find_by(original_book_id: original_book.id, user_id: current_user.id)
      # statusをupdate
      colored_book.update_attributes!(status: params[:status])
      # read関連のamountを更新
      colored_book.update_status_amount(original_book)

      # ranking用のデータを更新
      original_book_ranking.update_amount(original_book, original_book_ranking)

      if params[:kind] == "review"
        redirect_to new_book_review_path(id: colored_book.id)
      else
      ...
      colored_book = ColoredBook.new(original_book_id: original_book.id, status: params[:status], user_id: current_user.id)
      if colored_book.save
        # read関連のamountを更新
        colored_book.update_status_amount(original_book)
        # ranking用のデータを更新
        original_book_ranking.update_amount(original_book, original_book_ranking)
